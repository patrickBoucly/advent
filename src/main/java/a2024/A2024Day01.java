...
public class A2024Day01 extends A2024 {

	public A2024Day01(int day) {
		super(day);
	}

	public static void main(String[] args0) {
		A2024Day01 d = new A2024Day01(1);
	...
	public int s1(boolean b) {
		...
		for(String s:inputL) {
			left.add(Integer.parseInt(s.split(" ")[0].trim()));
			right.add(Integer.parseInt(s.split(" ")[1].trim()));
		}
	...
	public int s2(boolean b) {
		...
		int res=0;
		for(int i=0;i<left.size();i++) {
			res+=getNbOcc(right,left.get(i))*left.get(i);
		}
		return res;

	}
	
	private int getNbOcc(List<Integer> right, Integer i) {
		int occ=0;
